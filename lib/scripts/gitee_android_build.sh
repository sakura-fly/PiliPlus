...
if [ -x "$FLUTTER_ROOT/bin/flutter" ] && git -C "$FLUTTER_ROOT" rev-parse --git-dir >/dev/null 2>&1; then
  # 命中有效的 Flutter SDK（含可执行文件与 git 元数据），跳过克隆
  echo "使用已缓存的 Flutter SDK：$FLUTTER_ROOT"
else
  # 不直接 rm 缓存目录：缓存注入的目录可能被占用/只读，rm 会报 Device or resource busy。
  # 改为克隆到临时目录，再整体替换；替换失败则本次直接用临时目录继续
  FLUTTER_TMP="$HOME/.flutter_tmp"
  rm -rf "$FLUTTER_TMP" 2>/dev/null || true
  ...
  do
    ...
    if timeout 300 git clone -q --depth 1 -b "${FLUTTER_VERSION}" "$url" "$FLUTTER_TMP"; then
      FLUTTER_CLONE_OK=1
      break
    fi
  ...
  if [ -d "$FLUTTER_ROOT" ]; then
    rm -rf "${FLUTTER_ROOT}.old" 2>/dev/null || true
    mv "$FLUTTER_ROOT" "${FLUTTER_ROOT}.old" 2>/dev/null || true
  fi
  # 放入标准路径；若标准路径仍被占用，本次改用临时目录继续
  if ! mv "$FLUTTER_TMP" "$FLUTTER_ROOT" 2>/dev/null; then
    echo "警告：无法替换 $FLUTTER_ROOT（可能被缓存占用），本次使用 $FLUTTER_TMP"
    FLUTTER_ROOT="$FLUTTER_TMP"
    export FLUTTER_ROOT
  fi
fi
